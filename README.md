...
| `stateReadCacheSizeBytes` / `PROSODY_STATE_READ_CACHE_SIZE_BYTES` | Byte budget for the published-state read-through cache; must be a positive safe integer                                                                                                                                                | state cache size    |
...
```js
const carts = await client.state("carts", CART);
